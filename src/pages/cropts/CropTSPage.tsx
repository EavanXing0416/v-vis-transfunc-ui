...
import { buildCropTSPayload } from '../../features/cropts/buildCropTSPayload';
import { buildCropTSReadmes } from '../../features/cropts/buildCropTSReadmes';
import { buildDefaultCropTSForm } from '../../features/cropts/cropTS.defaults';
import { buildCropTSMetadataSummary, getCropTSOutputObjectCount } from '../../features/cropts/cropTSMetadata';
import type { CropTSFormState } from '../../features/cropts/cropTS.types';
import { getCropTSValidationMessage } from '../../features/cropts/cropTS.validation';
...
import { CropTSForm } from './components/CropTSForm';
import { CropTSReview } from './components/CropTSReview';

interface CropTSLocationState {
  selectedDatasets?: DatasetRecord[];
}

export function CropTSPage() {
  const location = useLocation();
  const locationState = location.state as CropTSLocationState | null;
  const dataset = locationState?.selectedDatasets?.[0] ?? mockDatasets[0];

  const [form, setForm] = useState(() => buildDefaultCropTSForm(dataset));
  const [commitNote, setCommitNote] = useState('');
  const [generatedFiles, setGeneratedFiles] = useState<string[]>([]);
  const [outputName, setOutputName] = useState(`${dataset.name}_crop`);
  const [outputType, setOutputType] = useState<DatasetRecord['type']>('virtual');

  useEffect(() => {
    setForm(buildDefaultCropTSForm(dataset));
    setOutputName(`${dataset.name}_crop`);
  }, [dataset]);

  const validationMessage = useMemo(() => getCropTSValidationMessage(form), [form]);
  const derivedDataset = useMemo<DerivedDatasetDraft>(() => ({
    role: 'selected',
    draft_id: `DRV-${dataset.id}-crop`,
    assigned_id: null,
    parent_id: dataset.id,
    name: outputName,
    type: outputType,
    object_count: getCropTSOutputObjectCount(dataset),
  }), [dataset, outputName, outputType]);
  const previewReadmes = useMemo(() => {
    const payload = buildCropTSPayload(dataset, form, derivedDataset);
    return buildCropTSReadmes(payload);
  ...
  return (
    <main className="app-shell">
      <PageHeader
        title="CropTS Configuration"
        description="Review the input dataset, detect the flat-top interval, crop aligned time-series signals, define the output dataset, and commit the transformation record."
        meta={
      ...
      <section className="grid grid--editor">
        <div className="panel">
          ...
          <div className="panel__section panel__section--compact">
            ...
            <CropTSForm form={form} onChange={setForm} validationMessage={validationMessage} />
          </div>
          ...
          <div className="panel__section panel__section--compact">
            ...
            <div className="dataset-summary-table dataset-summary-table--aligned">
              ...
              <article className="dataset-summary-row" key={derivedDataset.draft_id}>
                ...
                <span>{buildCropTSMetadataSummary(form)}</span>
              </article>
            </div>
          </div>
          ...
          <div className="panel__section panel__section--compact">
            ...
            <CommentEditor form={form} onChange={setForm as (nextForm: CropTSFormState) => void} />
          </div>
        </div>
        ...
        <aside className="panel">
          <div className="panel__section panel__section--compact">
            ...
            <CropTSReview dataset={dataset} derivedDataset={derivedDataset} form={form} />
