...
export function buildCropTSMetadataSummary(form: CropTSFormState) {
  ...
  return `Cropped time-series signals. Each data object represents one shot. Flat-top detection on ip with threshold ${form.ipThresholdKa} kA and flattop_min_frac = ${form.flattopMinFrac}. Trim start = ${startTrim}, trim end = ${endTrim}.`;
}
